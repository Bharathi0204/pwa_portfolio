...
export const AchievementsPage: React.FC = () => {
  ...
  return (
    <section id="achievements" className="py-12 sm:py-20 relative ambient-glow-cyan">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 relative z-10">
        ...
        <div 
          ...
          className="glass-panel p-4 sm:p-8 border-amber-500/50 bg-gradient-to-r from-slate-950 via-slate-900 to-amber-950/40 shadow-xl cursor-pointer group relative overflow-hidden mb-8 sm:mb-10"
        >
          {/* Confetti Trigger Badge */}
          <div className="flex sm:absolute sm:top-4 sm:right-4 mb-3 sm:mb-0 w-fit px-2.5 sm:px-3 py-1 rounded-full bg-amber-500/25 text-amber-300 border border-amber-500/50 text-[10px] sm:text-[11px] font-mono font-bold items-center gap-1.5 shadow-md">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 items-center">
            ...
            <div className="lg:col-span-4 flex items-center justify-center lg:justify-end pt-1 lg:pt-0">
              <div className="w-20 h-20 sm:w-28 sm:h-28 rounded-2xl bg-gradient-to-br from-amber-300 via-yellow-500 to-orange-600 p-0.5 shadow-xl flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-10">
          {PERSONAL_INFO.education.map((edu, idx) => (
            <div key={idx} className="glass-panel p-4 sm:p-6 relative overflow-hidden h-full border-white/10 hover:border-cyan-500/40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          {ACHIEVEMENTS.slice(2).map((item) => (
            <div
              key={item.id}
              className="p-4 sm:p-5 rounded-2xl bg-slate-950/70 border border-white/10 hover:border-cyan-500/30 transition-colors flex items-start gap-3 sm:gap-4 h-full"
            >
