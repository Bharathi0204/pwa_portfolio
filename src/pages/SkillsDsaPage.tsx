...
export const SkillsDsaPage: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-14 sm:py-20 relative">
      ...
      {/* Glow */}
      <div className="glow-orb glow-orb-violet w-[300px] sm:w-[400px] h-[300px] sm:h-[400px] top-10 left-10" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Header */}
        <div className="max-w-3xl mb-10 sm:mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/30 text-violet-300 text-[11px] sm:text-xs font-mono font-bold uppercase tracking-wider mb-2.5 sm:mb-3">
        ...
        <div className="glass-panel p-4 sm:p-6 mb-10 sm:mb-12">
          
          {/* Scrollable / wrap tabs */}
          <div className="flex flex-wrap gap-1.5 sm:gap-2 pb-4 sm:pb-6 border-b border-white/10 mb-4 sm:mb-6">
            {tabList.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`px-3 sm:px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 sm:gap-2 transition-all shrink-0 ${
                    isActive
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            {getActiveSkills().map((skill: any, idx: number) => (
              <div
                key={idx}
                className="p-3.5 sm:p-4 rounded-xl bg-slate-950/70 border border-white/5 hover:border-violet-500/30 transition-all flex flex-col justify-between group"
              >
                <div>
