...
import { CheckCircle2, ChevronRight, Sparkles, Terminal, Flame } from 'lucide-react';
...
export const DsaVisualizer: React.FC = () => {
  ...
  return (
    <div className="glass-panel p-4 sm:p-8 space-y-5 sm:space-y-6 border-white/10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-5 border-b border-white/10">
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-300 text-[10px] sm:text-xs font-mono font-bold uppercase flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-4 sm:flex items-center gap-2 sm:gap-3 bg-slate-950/80 p-2.5 sm:p-3 rounded-xl border border-white/5">
          <div className="text-center px-1 sm:px-3 sm:border-r border-white/10">
            <div className="text-[9px] sm:text-[10px] text-slate-400 uppercase font-mono">Total</div>
            <div className="font-heading font-black text-sm sm:text-xl text-white">{DSA_STATS.totalSolved}</div>
          </div>
          <div className="text-center px-1 sm:px-2 sm:border-r border-white/10">
            ...
            <div className="font-heading font-bold text-xs sm:text-base text-emerald-400">{DSA_STATS.easy}</div>
          </div>
          <div className="text-center px-1 sm:px-2 sm:border-r border-white/10">
            <div className="text-[9px] sm:text-[10px] text-amber-400 uppercase font-mono">Med</div>
            <div className="font-heading font-bold text-xs sm:text-base text-amber-400">{DSA_STATS.medium}</div>
          </div>
          <div className="text-center px-1 sm:px-2">
            <div className="text-[9px] sm:text-[10px] text-rose-400 uppercase font-mono">Hard</div>
            <div className="font-heading font-bold text-xs sm:text-base text-rose-400">{DSA_STATS.hard}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
        ...
        {/* Pattern List (Left 5 Cols) - Horizontal chips on small mobile, list on tablet/desktop */}
        <div className="lg:col-span-5 space-y-2">
          <div className="text-[10px] sm:text-xs font-mono text-slate-400 uppercase tracking-wider mb-2">
              ...
              return (
                ...
                >
                  <div className="min-w-0 pr-2">
                    ...
                    <div className="text-[9px] sm:text-[11px] text-slate-400 font-mono mt-0.5">
                      {pattern.count} Problems Mastered
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span className={`px-2 py-0.5 rounded text-[9px] sm:text-[10px] font-mono ${
                      isSelected ? 'bg-cyan-500/20 text-cyan-300' : 'bg-slate-900 text-slate-400'
                    }`}>
                      {Math.round((pattern.count / DSA_STATS.totalSolved) * 100)}%
                    </span>
                    <ChevronRight className={`w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 ${isSelected ? 'text-cyan-400' : 'text-slate-600'}`} />
                  </div>
                </button>
              );
            })}
        ...
        <div className="lg:col-span-7">
          <Tilt3DCard maxTilt={4} className="w-full h-full">
            <div className="bg-slate-950/90 rounded-2xl border border-cyan-500/30 p-4 sm:p-6 flex flex-col justify-between h-full">
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 font-mono text-[10px] sm:text-xs">
                    {DSA_PATTERNS[selectedPattern].count} Problems
                  </span>
                </div>

                <h4 className="font-heading font-bold text-base sm:text-xl text-white mb-2">
              ...
              <div className="mt-4 pt-3 border-t border-white/10 flex flex-wrap items-center justify-between gap-2 text-[10px] sm:text-xs text-slate-400 font-mono">
