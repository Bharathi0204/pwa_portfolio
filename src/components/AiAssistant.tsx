...
export const AiAssistant: React.FC<AiAssistantProps> = ({ isOpen, onClose }) => {
  ...
  // Helper to format markdown-like bold text and lists cleanly
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div 
        className="relative w-full max-w-2xl h-[85vh] bg-slate-900 border border-violet-500/40 rounded-2xl shadow-2xl shadow-violet-500/20 text-slate-100 overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-white/10 bg-slate-950/80 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-600 to-cyan-500 p-0.5 shadow-lg shadow-violet-500/30">
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4 custom-scrollbar">
            ...
            return (
              ...
              >
                ...
                <div className={`max-w-[85%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed ${
                  ...
                  <div className={`text-[10px] mt-2 font-mono ${isBot ? 'text-slate-500' : 'text-cyan-200'} text-right`}>
                    {msg.timestamp}
                  </div>
                </div>
              </div>
          ...
          {isTyping && (
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-violet-600/20 text-violet-300 border border-violet-500/30 flex items-center justify-center">
                <Bot className="w-4 h-4 animate-spin" />
              </div>
              <div className="p-3 rounded-xl bg-slate-950/80 border border-white/10 text-xs text-slate-400 flex items-center gap-1.5 font-mono">
        ...
        {suggestedChips.length > 0 && (
          <div className="px-4 py-2 bg-slate-950/60 border-t border-white/5 overflow-x-auto flex items-center gap-2 no-scrollbar">
            ...
            {suggestedChips.map((chip, idx) => (
              <button
                ...
                className="whitespace-nowrap px-3 py-1 rounded-full bg-slate-900 hover:bg-violet-950/60 border border-white/10 hover:border-violet-400/50 text-[11px] text-slate-300 hover:text-cyan-300 transition-all shrink-0"
              >
                {chip}
              </button>
            ))}
        ...
        <div className="p-4 border-t border-white/10 bg-slate-950 flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-slate-900 border border-white/10 focus:border-cyan-400 rounded-xl px-4 py-3 text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none transition-colors"
          />
          <button
            onClick={() => handleSend()}
            disabled={!inputText.trim()}
            className="p-3 rounded-xl bg-gradient-to-r from-cyan-500 to-sky-500 text-slate-950 font-bold hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-md shadow-cyan-500/20"
          >
